...
- **Full image crashes at startup without a GPU** (`libcuda.so.1: cannot open shared object file`, raised from the llama_cpp import): the full image requires CUDA for code search. On machines without an NVIDIA GPU (laptops, CI), use the `-slim` image variant, which is also what the hosted deployments run.
...
- **Gemini CLI headless**: needs `GEMINI_CLI_TRUST_WORKSPACE=true` and `--allowed-mcp-server-names plainsight` in addition to the `mcpServers` settings entry (see `docs/demo/.gemini/settings.json`).
